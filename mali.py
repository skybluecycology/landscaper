...
def load_csv_to_dataclass(filename, dataclass_type):
    with open(filename) as csv_file:
        ...
        for row in reader:
            ...
            missing_fields = [field.name for field in fields(dataclass_type) if field.name not in row]
            if missing_fields:
            ...
            print(f"Field data before conversion: {field_data}")  # Debugging output
            ...
            for field in fields(dataclass_type):
                if field.name in field_data:
                    try:
                        ...
                        print(f"Error converting {field.name}: {e}")

            print(f"Field data after conversion: {field_data}")  # Debugging output
